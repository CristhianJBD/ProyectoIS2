...
class DeleteUsSprintView(LoginRequiredMixin, ActiveProjectRequiredMixin, generic.FormView):
    """
    Vista eliminacion de User Stories en sprint
    """
    ...
    class UpdateSprintView(ActiveProjectRequiredMixin, LoginRequiredMixin, GlobalPermissionRequiredMixin,
                           generic.UpdateView):
        """
        Vista para actualizar los datos del Sprint y  del UserStory que son el desarrollador, la actividad y el Sprint
        """
        ...
        def __filtrar_formset__(self, formset):
            for userformset in formset.forms:
                userformset.fields['desarrollador'].queryset = User.objects.filter(
                    miembroequiposprint__sprint=self.object)
                userformset.fields['flujo'].queryset = Flujo.objects.filter(proyecto=self.get_proyecto())
                userformset.fields['userStory'].queryset = UserStory.objects.filter(Q(proyecto=self.get_proyecto()),
                                                                                    Q(estado=0)).order_by(
                    '-prioridadFormula')
...
class Reasignar(ActiveProjectRequiredMixin, LoginRequiredMixin, GlobalPermissionRequiredMixin, generic.UpdateView):
    """
        Reasigna desarrollador
    """
    model = Sprint
    permission_required = 'administracion.editar_sprint'
    template_name = 'administracion/sprint/sprint_form_delete.html'
    form_class = modelform_factory(Sprint,form=AddSprintBaseForm,
                                   widgets={'fecha_inicio': SelectDateWidget,'proyecto': HiddenInput},
                                   fields={'nombre', 'fecha_inicio','duracion_sprint','estado','proyecto'})
    UserStoryFormset = formset_factory(AddToSprintForm, formset=AddToSprintFormset, can_delete=True, extra=0)
    formset = None
